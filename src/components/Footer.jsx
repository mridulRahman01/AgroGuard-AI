        ...
        <div className="container-main py-12 md:py-16">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
                ...
                <div className="lg:col-span-1 space-y-5">
                    ...
                    <p className="text-green-200 text-sm leading-relaxed" style={{ fontFamily: 'Hind Siliguri, sans-serif' }}>
                    ...
                    <div className="flex gap-3 pt-2">
                        {[Facebook, Twitter, Youtube].map((Icon, i) => (
                            <a
                                key={i}
                                href="#"
                                className="w-9 h-9 rounded-xl bg-green-700/50 hover:bg-green-500 flex items-center justify-center transition-all duration-300 hover:scale-110"
                            >
                ...
                {/* Quick Links */}
                <div>
                    <h3 className="font-semibold text-green-300 mb-5 text-sm uppercase tracking-wider" style={{ fontFamily: 'Hind Siliguri, sans-serif' }}>
                        দ্রুত লিংক
                    </h3>
                    <ul className="space-y-3">
                        {QUICK_LINKS.map(({ to, label }) => (
                            <li key={to}>
                                <Link
                                    to={to}
                                    className="text-green-200 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2.5 group w-fit"
                                    style={{ fontFamily: 'Hind Siliguri, sans-serif' }}
                                >
                                    <span className="w-1.5 h-1.5 rounded-full bg-green-500/50 group-hover:bg-green-400 transition-colors flex-shrink-0" />
                                    {label}
                                </Link>
                            </li>
                        ))}
                ...
                <div>
                    ...
                    <ul className="space-y-3.5">
                        {CONTACT_INFO.map(({ icon: Icon, text }) => (
                            <li key={text} className="flex items-start gap-3 w-fit hover:text-white transition-colors cursor-default">
                                <span className="w-6 h-6 rounded-lg bg-green-800/50 flex flex-shrink-0 flex-col items-center justify-center">
                                    <Icon className="w-3.5 h-3.5 text-green-400" />
                                </span>
                                <span className="text-green-200 text-sm mt-0.5" style={{ fontFamily: 'Hind Siliguri, sans-serif' }}>{text}</span>
                            </li>
                        ))}
                    </ul>
                </div>
